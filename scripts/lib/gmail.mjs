 ...
 */
export function gmailLink(messageId, accountEmail) {
  // /u/0/ is a best-effort guess at the account slot; Gmail redirects to the
  // right one if the user is signed into several.
  return `https://mail.google.com/mail/u/0/#all/${messageId}${accountEmail ? `?authuser=${encodeURIComponent(accountEmail)}` : ''}`;
}
